...
const registerUser = asyncHandler(async (req, res) => {
  //avatar comes from multer and hence we can not get it from req.body
  ...
  // with $or we can check for multiple condition in find method
  ...
  //.files only comes after adding multer
  //if files exist -> check avatar[0] which provides an object -> .path a variable in object to access path of image
  ...
  if (!avatarUploaded?.url || !avatarUploaded?.public_id) {
    throw new ApiError(400, "Avatar is required failed to upload on cloudinary");
  }

  const coverImageUploaded = coverImageLocalPath
    ? await uploadOnCloudinary(coverImageLocalPath)
    : { url: "", public_id: "" };

  if (
    coverImageLocalPath &&
    (!coverImageUploaded?.url || !coverImageUploaded?.public_id)
  ) {
  ...
  //to check if registered or not we can also check in db directly by a query to find record

  //to select some fields we can use select() method
  //by default all fields are selected and we have to write which field not to select
  const isUser = await User.findById(user._id).select("-password -refreshToken");

  if (!isUser) throw new ApiError(500, "User not registered problem in insertion in db");
